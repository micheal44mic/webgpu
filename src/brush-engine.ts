...
import {
  abandonRasterNoiseSession,
  beginRasterNoise as beginRasterNoiseRuntime,
  cancelRasterNoise as cancelRasterNoiseRuntime,
  commitRasterNoise as commitRasterNoiseRuntime,
  updateRasterNoise as updateRasterNoiseRuntime,
  type ActiveRasterNoiseSession,
...
import {
  abandonRasterColorAdjustSession,
  beginRasterColorAdjust as beginRasterColorAdjustRuntime,
  cancelRasterColorAdjust as cancelRasterColorAdjustRuntime,
  commitRasterColorAdjust as commitRasterColorAdjustRuntime,
  prewarmRasterColorAdjustRuntime,
  updateRasterColorAdjust as updateRasterColorAdjustRuntime,
  type ActiveRasterColorAdjustSession,
} from "./engine-raster-color-adjust-runtime";
import type { RasterColorAdjustSettings } from "./raster-color-adjust-core.ts";
import {
  abandonRasterColorBalanceSession,
  beginRasterColorBalance as beginRasterColorBalanceRuntime,
  cancelRasterColorBalance as cancelRasterColorBalanceRuntime,
  commitRasterColorBalance as commitRasterColorBalanceRuntime,
  prewarmRasterColorBalanceRuntime,
  updateRasterColorBalance as updateRasterColorBalanceRuntime,
  type ActiveRasterColorBalanceSession,
...
export class BrushEngine {
  ...
  private rasterColorAdjustPipelinesReady = false;
  ...
  /** Prewarms the lightweight Color Adjust pipeline without opening an edit. */
  async prewarmRasterColorAdjustResources(): Promise<void> {
    if (this.rasterColorAdjustPipelinesReady) return;
    if (this.rasterColorAdjustPrewarmPromise) {
      await this.rasterColorAdjustPrewarmPromise;
      return;
    }
    const initialization = (async (): Promise<void> => {
    ...
    this.rasterColorAdjustPrewarmPromise = initialization;
    try {
      await initialization;
    } finally {
      if (this.rasterColorAdjustPrewarmPromise === initialization) {
        this.rasterColorAdjustPrewarmPromise = null;
      }
    }
  }

  /** Prewarms the lightweight Color Balance pipeline without opening an edit. */
  async prewarmRasterColorBalanceResources(): Promise<void> {
    if (this.rasterColorBalancePipelinesReady) return;
    if (this.rasterColorBalancePrewarmPromise) {
      await this.rasterColorBalancePrewarmPromise;
      return;
    }
    const initialization = (async (): Promise<void> => {
    ...
    this.rasterColorBalancePrewarmPromise = initialization;
    try {
      await initialization;
    } finally {
      if (this.rasterColorBalancePrewarmPromise === initialization) {
        this.rasterColorBalancePrewarmPromise = null;
      }
    }
  }
  ...
  beginRasterNoise(initial?: Partial<RasterNoiseSettings>) {
    return beginRasterNoiseRuntime(this, initial);
  }

  updateRasterNoise(update: Partial<RasterNoiseSettings>) {
    return updateRasterNoiseRuntime(this, update);
  }

  commitRasterNoise(): Promise<boolean> {
    return commitRasterNoiseRuntime(this);
  }
  ...
  beginRasterColorAdjust(initial?: Partial<RasterColorAdjustSettings>) {
    return beginRasterColorAdjustRuntime(this, initial);
  }

  updateRasterColorAdjust(update: Partial<RasterColorAdjustSettings>) {
    return updateRasterColorAdjustRuntime(this, update);
  }

  commitRasterColorAdjust(): Promise<boolean> {
    return commitRasterColorAdjustRuntime(this);
  }

  cancelRasterColorAdjust(): Promise<boolean> {
    return cancelRasterColorAdjustRuntime(this);
  }

  beginRasterColorBalance(initial?: Partial<RasterColorBalanceSettings>) {
    return beginRasterColorBalanceRuntime(this, initial);
  }

  updateRasterColorBalance(update: Partial<RasterColorBalanceSettings>) {
    return updateRasterColorBalanceRuntime(this, update);
  }

  commitRasterColorBalance(): Promise<boolean> {
    return commitRasterColorBalanceRuntime(this);
  }
  ...
  assertLayerSwitchAllowed(): void {
    ...
    if (this.activeRasterColorAdjustSession) {
      throw new Error(
        "Apply or cancel Color Adjust before switching layers.",
      );
    }
