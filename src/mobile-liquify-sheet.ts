...
// Liquify keeps the mode rail and the first controls reachable in peek while
// leaving enough canvas visible to deform with one finger. The expanded state
// exposes the complete parameter set without changing the active GPU session.
...
export function mobileLiquifyPeekHeight(viewportHeight: number): number {
  return Math.min(
    MOBILE_LIQUIFY_MAX_PEEK_PX,
    Math.max(
      MOBILE_LIQUIFY_MIN_PEEK_PX,
      viewportHeight * MOBILE_LIQUIFY_PEEK_VIEWPORT_RATIO,
    ),
  );
}

/**
 * Presentation-only three-detent sheet for the transactional Liquify editor.
 * The engine owns pixels and rollback. Dismissing the sheet always requests
...
export class MobileLiquifySheetController {
  ...
  get currentSnap(): MobileBottomSheetSnap {
    return this.snap;
  }
  ...
  private snapTo(snap: MobileBottomSheetSnap): void {
    ...
    this.handle.setAttribute(
      "aria-label",
      `${minimized ? "Restore" : expanded ? "Collapse" : "Expand"} Liquify settings`,
    );
    this.setOffset(
