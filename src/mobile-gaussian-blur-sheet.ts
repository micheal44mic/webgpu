...
function peekHeight(viewportHeight: number): number {
  return Math.min(
    MOBILE_GAUSSIAN_BLUR_MAX_PEEK_PX,
    Math.max(
      MOBILE_GAUSSIAN_BLUR_MIN_PEEK_PX,
      viewportHeight * MOBILE_GAUSSIAN_BLUR_PEEK_VIEWPORT_RATIO,
    ),
  );
}

/**
 * Presentation-only bottom sheet for the destructive Gaussian Blur session.
 * BrushEngine remains the sole owner of preview pixels, commit and rollback;
...
export class MobileGaussianBlurSheetController {
  ...
  private snapTo(snap: MobileBottomSheetSnap): void {
    ...
    this.handle.setAttribute(
      "aria-label",
      `${minimized ? "Restore" : expanded ? "Collapse" : "Expand"} Gaussian Blur settings`,
    );
    this.setOffset(
