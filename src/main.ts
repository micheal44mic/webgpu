...
async function replayHumanStroke(): Promise<void> {
  ...
  try {
    ...
    humanStrokeResult.textContent = [
      ...
      `submit→fine coda p95 ${performanceProfile.submissionCompletionP95Ms.toFixed(2)} ms`,
      `in-flight ${performanceProfile.peakInFlightSubmissions}/${performanceProfile.submissionLimit}`,
      `backlog max ${formatInteger(performanceProfile.maxPendingStamps)} stamp base`,
      `saturazioni ${formatInteger(performanceProfile.backpressureWaits)}`,
