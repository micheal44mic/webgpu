...
`);throw Error(`Errore WGSL nel modulo ${t}:\n${r}`)}describeAdapter(e){let t=e.info,n=[t.vendor,t.architecture,t.device,t.description].map(e=>e?.trim()).filter(e=>!!e);return[...new Set(n)].join(` · `)||`GPU WebGPU`}};function B(e){let t=document.getElementById(e);if(!t)throw Error(`Elemento #${e} non trovato.`);return t}function V(e){return Number(B(e).value)}function H(e){return new Intl.NumberFormat(`it-IT`,{maximumFractionDigits:0}).format(e)}var U=B(`gpuCanvas`),Cn=B(`tipPreviewCanvas`),wn=B(`controlsPanel`),Tn=B(`toggleControls`),En=B(`status`),Dn=B(`runBenchmark`),On=B(`benchmarkResult`),kn=B(`recordHumanStroke`),An=B(`playHumanStroke`),W=B(`humanStrokeResult`),jn=B(`humanStrokeTestVariant`),Mn=B(`humanStrokeTestBlendMode`),Nn=B(`humanStrokeTestGrainMode`),Pn=B(`layerFormat`),Fn=B(`clearLayer`),In=B(`undoStroke`),Ln=B(`redoStroke`),Rn=B(`fitView`),zn=B(`zoomIn`),Bn=B(`zoomOut`),Vn=B(`benchmarkStamps`),Hn=`webgpu-brush-engine.human-stroke.v1`,Un=`/api/human-stroke`,Wn=`/api/benchmark-runs`,G={canUndo:!1,canRedo:!1,busy:!1,actionCount:0,cursor:0,storedBaseStamps:0,logicalStampBytes:0},K=new Sn(U,{onStatus(e,t){En.textContent=e,En.className=`status ${t===`working`?``:t}`},onStats(e){Ir(e)},onHistoryChange(e){G=e,Z(),Y()}},Cn),q=null,Gn=null,Kn=!1,qn=!1,Jn=!0,Yn=!1,Xn=!1,Zn=!1,Qn=!1,$n=!1,er=!0,tr=`paint`,nr={paint:{size:96,spacing:1,flow:7,hardness:88},blend:{size:100,spacing:10,flow:45,hardness:8}};function rr(){nr[tr]={size:V(`brushSize`),spacing:V(`spacing`),flow:V(`flow`),hardness:V(`hardness`)}}function ir(e,t){let n=tr;t&&n!==e&&rr(),tr=e,J(`brushTool`,e);let r=e===`blend`,i=B(`brushSize`),a=B(`spacing`);if(i.min=r?`1`:`4`,i.max=r?`1024`:`1500`,a.min=r?`1`:`0.25`,a.max=r?`400`:`25`,a.step=r?`1`:`0.25`,t&&n!==e){let t=nr[e];J(`brushSize`,t.size),J(`spacing`,t.spacing),J(`flow`,t.flow),J(`hardness`,t.hardness)}for(let e of[`shapeScatterControl`,`countControl`,`opacityControl`,`paintBlendIntensityControl`,`paintBlendModeControl`,`thicknessSection`,`colorJitterSection`,`positionJitterSection`])B(e).hidden=r;B(`blendControls`).hidden=!r}function ar(e){er=e,wn.hidden=!e,Tn.setAttribute(`aria-expanded`,String(e)),Tn.setAttribute(`aria-label`,e?`Nascondi pannelli`:`Mostra pannelli`),Tn.title=e?`Nascondi pannelli`:`Mostra pannelli`}function or(){return{tool:B(`brushTool`).value===`blend`?`blend`:`paint`,shape:B(`brushShape`).value,shapeScatter:V(`shapeScatter`)/100,grainMode:B(`grainMode`).value,grainScale:V(`grainScale`)/100,grainDepth:V(`grainDepth`)/100,grainBrightness:V(`grainBrightness`)/100,grainContrast:V(`grainContrast`)/100,grainInvert:B(`grainInvert`).checked,grainFiltering:B(`grainFiltering`).value,grainBlendMode:B(`grainBlendMode`).value,color:B(`brushColor`).value,size:V(`brushSize`),spacingPercent:V(`spacing`),startThickness:V(`startThickness`)/100,endThickness:V(`endThickness`)/100,count:V(`count`),flow:V(`flow`)/100,opacity:V(`opacity`)/100,hardness:V(`hardness`)/100,blendIntensity:V(`blendIntensity`),blendMode:B(`blendMode`).value,blendStretch:V(`blendStretch`)/100,blendPaint:V(`blendPaint`)/100,jitterMaster:V(`jitterMaster`)/100,hueJitterDegrees:V(`hueJitter`),saturationJitter:V(`saturationJitter`)/100,lightnessJitter:V(`lightnessJitter`)/100,darknessJitter:V(`darknessJitter`)/100,jitterPerCopy:B(`jitterPerCopy`).checked,positionJitterLateral:V(`positionJitterLateral`)/100,positionJitterLinear:V(`positionJitterLinear`)/100}}function sr(){B(`shapeScatterOut`).value=`${V(`shapeScatter`).toFixed(0)}%`,B(`grainScaleOut`).value=`${V(`grainScale`).toFixed(0)}%`,B(`grainDepthOut`).value=`${V(`grainDepth`).toFixed(0)}%`;let e=V(`grainBrightness`);B(`grainBrightnessOut`).value=`${e>0?`+`:``}${e.toFixed(0)}%`;let t=V(`grainContrast`);B(`grainContrastOut`).value=`${t>0?`+`:``}${t.toFixed(0)}%`,B(`brushSizeOut`).value=`${V(`brushSize`).toFixed(0)} px`,B(`spacingOut`).value=`${V(`spacing`).toFixed(2)}%`,B(`startThicknessOut`).value=`${V(`startThickness`).toFixed(0)}%`,B(`endThicknessOut`).value=`${V(`endThickness`).toFixed(0)}%`,B(`countOut`).value=V(`count`).toFixed(0),B(`flowOut`).value=`${V(`flow`).toFixed(1).replace(`.0`,``)}%`,B(`opacityOut`).value=`${V(`opacity`).toFixed(1).replace(`.0`,``)}%`,B(`hardnessOut`).value=`${V(`hardness`).toFixed(0)}%`,B(`blendIntensityOut`).value=`${V(`blendIntensity`).toFixed(2)}×`,B(`blendStretchOut`).value=`${V(`blendStretch`).toFixed(0)}%`,B(`blendPaintOut`).value=`${V(`blendPaint`).toFixed(0)}%`,B(`jitterMasterOut`).value=`${V(`jitterMaster`).toFixed(0)}%`,B(`hueJitterOut`).value=`${V(`hueJitter`).toFixed(0)}°`,B(`saturationJitterOut`).value=`${V(`saturationJitter`).toFixed(0)}%`,B(`lightnessJitterOut`).value=`${V(`lightnessJitter`).toFixed(0)}%`,B(`darknessJitterOut`).value=`${V(`darknessJitter`).toFixed(0)}%`,B(`positionJitterLateralOut`).value=`${V(`positionJitterLateral`).toFixed(0)}%`,B(`positionJitterLinearOut`).value=`${V(`positionJitterLinear`).toFixed(0)}%`,B(`benchmarkStampsOut`).value=H(V(`benchmarkStamps`))}function cr(){rr(),sr(),Pr(),K.setBrushSettings(or())}function lr(e){return`${(e/1e3).toFixed(2)} s`}function ur(e,t){if(e.length===0)return 0;let n=[...e].sort((e,t)=>e-t);return n[Math.min(n.length-1,Math.max(0,Math.ceil(n.length*t)-1))]}function dr(){return new Promise(e=>requestAnimationFrame(e))}function fr(e){let t=2166136261;for(let n of e)for(let e of[Math.round(n.x*10),Math.round(n.y*10),Math.round(n.pressure*1e3),Math.round(n.timeMs*10)])t=Math.imul(t^e,16777619)>>>0;return t.toString(16).padStart(8,`0`)}function pr(e){let t=0,n=0,r=[];for(let i=1;i<e.length;i+=1){let a=e[i-1],o=e[i],s=Math.max(0,o.timeMs-a.timeMs),c=Math.hypot(o.x-a.x,o.y-a.y);t+=c,r.push(s),s>0&&(n=Math.max(n,c/s*1e3))}let i=e.at(-1)?.timeMs??0;return{pathLengthPx:t,averageSpeedPxPerSecond:i>0?t/i*1e3:0,peakSpeedPxPerSecond:n,sampleGapP95Ms:ur(r,.95),sampleGapMaxMs:r.length===0?0:Math.max(...r),inputGapsOver33Ms:r.filter(e=>e>33).length}}function mr(){let e=navigator,t=K.getBenchmarkEnvironment();return{userAgent:navigator.userAgent,platform:navigator.platform,language:navigator.language,maxTouchPoints:navigator.maxTouchPoints,devicePixelRatio:window.devicePixelRatio||1,screenWidth:window.screen.width,screenHeight:window.screen.height,viewportWidth:window.innerWidth,viewportHeight:window.innerHeight,hardwareConcurrency:navigator.hardwareConcurrency||null,deviceMemoryGiB:e.deviceMemory??null,connection:e.connection?.effectiveType??e.connection?.type??null,controlsLayoutStrategy:`full-stage-overlay-drawer`,touchNavigationStrategy:`two-finger-pan-pinch`,performanceTelemetryRevision:29,...t}}async function hr(e){let t=await fetch(Wn,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(e)});if(!t.ok)throw Error(`Il risultato è visibile, ma non è stato possibile aggiungerlo al registro.`);let n=await t.json();return typeof n.id==`number`?n.id:0}function gr(e){if(typeof e!=`object`||!e)return null;let t=e;if(t.version!==1||!t.settings||!Array.isArray(t.points)||t.points.length===0)return null;let n=t,r=Number.isFinite(n.settings.opacity)?Math.min(1,Math.max(0,n.settings.opacity)):1,i=n.settings.blendMode===`additive`||n.settings.blendMode===`light-glaze`||n.settings.blendMode===`m1-glaze`?n.settings.blendMode:`normal`,a=n.settings.grainMode===`texturized`||n.settings.grainMode===`moving`?n.settings.grainMode:`off`,o=Number.isFinite(n.settings.grainScale)?Math.min(4,Math.max(.1,n.settings.grainScale)):1.4,s=Number.isFinite(n.settings.grainDepth)?Math.min(1,Math.max(0,n.settings.grainDepth)):1,c=Number.isFinite(n.settings.grainBrightness)?Math.min(1,Math.max(-1,n.settings.grainBrightness)):0,l=Number.isFinite(n.settings.grainContrast)?Math.min(1,Math.max(-1,n.settings.grainContrast)):0,u=n.settings.grainInvert===!0,d=n.settings.grainFiltering===`no`||n.settings.grainFiltering===`classic`?n.settings.grainFiltering:`improved`,f=Number.isFinite(n.settings.startThickness)?Math.min(2,Math.max(0,n.settings.startThickness)):1,p=Number.isFinite(n.settings.endThickness)?Math.min(2,Math.max(0,n.settings.endThickness)):1,m=Number.isFinite(n.settings.blendStretch)?Math.min(1,Math.max(0,n.settings.blendStretch)):.18,h=Number.isFinite(n.settings.blendPaint)?Math.min(1,Math.max(0,n.settings.blendPaint)):.14,g={...n.settings};return delete g.speedThickness,delete g.pressureSize,delete g.pressureOpacity,{...n,settings:{...g,tool:`paint`,shape:n.settings.shape===`shape`?`shape`:`circle`,shapeScatter:Number.isFinite(n.settings.shapeScatter)?Math.min(1,Math.max(0,n.settings.shapeScatter)):0,grainMode:a,grainScale:o,grainDepth:s,grainBrightness:c,grainContrast:l,grainInvert:u,grainFiltering:d,grainBlendMode:`multiply`,startThickness:f,endThickness:p,opacity:r,blendMode:i,blendStretch:m,blendPaint:h}}}function _r(){try{let e=window.localStorage.getItem(Hn);return e?gr(JSON.parse(e)):null}catch{return null}}function vr(){try{window.localStorage.removeItem(Hn)}catch{}}async function yr(){let e=await fetch(Un,{cache:`no-store`});if(e.status===404)return null;if(!e.ok)throw Error(`Impossibile caricare il tratto umano di riferimento.`);return gr(await e.json())}async function br(e){let t=await fetch(Un,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(e)});if(t.status===409){let e=gr(await t.json());if(e)return e}if(!t.ok)throw Error(`Impossibile fissare il tratto umano di riferimento.`);let n=gr(await t.json());if(!n)throw Error(`Il tratto umano salvato non è valido.`);return n}async function xr(){Jn=!0,Y();try{let e=await yr();if(e){q=e,vr(),W.textContent=Mr(e);return}let t=_r();if(t){W.textContent=`Fissaggio del tratto che avevi già registrato…`,q=await br(t),vr(),W.textContent=Mr(q);return}W.textContent=`Nessun tratto di riferimento: registralo una sola volta.`}catch(e){W.textContent=e instanceof Error?e.message:String(e)}finally{Jn=!1,Y()}}function J(e,t){B(e).value=String(t)}function Sr(e){ir(e.tool===`blend`?`blend`:`paint`,!1),J(`brushShape`,e.shape===`shape`?`shape`:`circle`),J(`shapeScatter`,(e.shapeScatter??0)*100),J(`grainMode`,e.grainMode===`texturized`||e.grainMode===`moving`?e.grainMode:`off`),J(`grainScale`,(e.grainScale??1.4)*100),J(`grainDepth`,(e.grainDepth??1)*100),J(`grainBrightness`,(e.grainBrightness??0)*100),J(`grainContrast`,(e.grainContrast??0)*100),B(`grainInvert`).checked=e.grainInvert===!0,J(`grainFiltering`,e.grainFiltering===`no`||e.grainFiltering===`classic`?e.grainFiltering:`improved`),J(`grainBlendMode`,`multiply`),J(`brushColor`,e.color),J(`brushSize`,e.size),J(`spacing`,e.spacingPercent),J(`startThickness`,(e.startThickness??1)*100),J(`endThickness`,(e.endThickness??1)*100),J(`count`,e.count),J(`flow`,e.flow*100),J(`opacity`,(e.opacity??1)*100),J(`hardness`,e.hardness*100),J(`blendIntensity`,e.blendIntensity),J(`blendMode`,e.blendMode),J(`blendStretch`,(e.blendStretch??.18)*100),J(`blendPaint`,(e.blendPaint??.14)*100),J(`jitterMaster`,e.jitterMaster*100),J(`hueJitter`,e.hueJitterDegrees),J(`saturationJitter`,e.saturationJitter*100),J(`lightnessJitter`,e.lightnessJitter*100),J(`darknessJitter`,e.darknessJitter*100),B(`jitterPerCopy`).checked=e.jitterPerCopy,J(`positionJitterLateral`,e.positionJitterLateral*100),J(`positionJitterLinear`,e.positionJitterLinear*100),cr()}function Cr(){return ir(`paint`,!1),J(`brushShape`,`circle`),J(`shapeScatter`,0),J(`grainMode`,`off`),J(`grainScale`,140),J(`grainDepth`,100),J(`grainBrightness`,0),J(`grainContrast`,0),B(`grainInvert`).checked=!1,J(`grainFiltering`,`improved`),J(`grainBlendMode`,`multiply`),J(`brushSize`,750),J(`spacing`,1),J(`startThickness`,100),J(`endThickness`,100),J(`count`,16),J(`flow`,100),J(`opacity`,100),J(`hardness`,100),J(`blendIntensity`,4),J(`blendMode`,`normal`),J(`blendStretch`,18),J(`blendPaint`,14),J(`jitterMaster`,100),J(`hueJitter`,180),J(`saturationJitter`,100),B(`jitterPerCopy`).checked=!0,J(`positionJitterLateral`,100),J(`positionJitterLinear`,100),cr(),or()}function wr(){return jn.value===`fur`?`fur`:`base`}function Tr(){return Mn.value===`m1-glaze`?`m1-glaze`:`normal`}function Er(){return Nn.value===`texturized`?`texturized`:`off`}function Dr(e,t,n,r){let i={...e.settings,tool:`paint`,opacity:1,blendIntensity:n===`m1-glaze`?1:4,blendMode:n,blendStretch:.18,blendPaint:.14,grainMode:r,grainScale:1.4,grainDepth:1,grainBrightness:0,grainContrast:0,grainInvert:!1,grainFiltering:`improved`,grainBlendMode:`multiply`,shape:`circle`,shapeScatter:0,startThickness:1,endThickness:1,positionJitterLateral:1,positionJitterLinear:1};return t===`fur`?{...i,shape:`shape`,shapeScatter:1,positionJitterLateral:0,positionJitterLinear:0}:i}function Or(e,t,n){return`${e===`fur`?`Fur`:`Base`} · ${t===`m1-glaze`?`M1 Glaze non accumulativo · 1×`:`Normal accumulativo · 4×`} · ${n===`texturized`?`Grain Fixed M1`:`Grain Off`}`}function Y(){let e=!$n||Qn||G.busy||Xn||Zn;kn.disabled=e||Jn||Yn||qn||!!q,kn.textContent=Kn?`Annulla registrazione tratto`:q?`Tratto umano fissato`:`Registra tratto umano`,An.disabled=e||!q||Jn||Yn||qn,jn.disabled=e||Jn||Yn||qn||Kn||!!Gn,Mn.disabled=e||Jn||Yn||qn||Kn||!!Gn,Nn.disabled=e||Jn||Yn||qn||Kn||!!Gn}function kr(){return!$n||Qn||G.busy||Xn||Zn||qn||Yn}function X(){return kr()||Q!==null}function Z(){let e=X();In.disabled=e||!G.canUndo,Ln.disabled=e||!G.canRedo,Fn.disabled=e;let t=tr===`blend`;Dn.disabled=e||t,Vn.disabled=e||t,Pn.disabled=e,Rn.disabled=e,zn.disabled=e,Bn.disabled=e,Tn.disabled=Xn||qn;for(let t of Fr)B(t).disabled=e;Pr(e)}async function Ar(e){if(!(X()||Q!==null)&&!(e===`undo`?!G.canUndo:!G.canRedo)){Qn=!0,Z(),Y();try{e===`undo`?await K.undo():await K.redo()}catch(e){En.textContent=e instanceof Error?e.message:String(e),En.className=`status error`}finally{Qn=!1,G=K.getHistoryState(),Z(),Y()}}}async function jr(){if(!(X()||Q!==null)){Qn=!0,Z(),Y();try{await K.clear()}catch(e){En.textContent=e instanceof Error?e.message:String(e),En.className=`status error`}finally{Qn=!1,G=K.getHistoryState(),Z(),Y()}}}function Mr(e){let t=e.points.at(-1)?.timeMs??0;return[`Tratto salvato: ${H(e.points.length)} campioni`,`durata ${lr(t)}`,`size ${e.settings.size.toFixed(0)} px`,`Count ${e.settings.count}`].join(` · `)}var Nr=[`grainScale`,`grainDepth`,`grainBrightness`,`grainContrast`,`grainInvert`,`grainFiltering`,`grainBlendMode`];function Pr(e=X()){let t=B(`grainMode`).value,n=t===`texturized`||t===`moving`;B(`grainParameters`).hidden=!n;for(let r of Nr)B(r).disabled=e||!n||r===`grainScale`&&t===`moving`}var Fr=[`brushTool`,`brushShape`,`shapeScatter`,`grainMode`,...Nr,`brushColor`,`brushSize`,`spacing`,`startThickness`,`endThickness`,`count`,`flow`,`opacity`,`hardness`,`blendIntensity`,`blendMode`,`blendStretch`,`blendPaint`,`jitterMaster`,`hueJitter`,`saturationJitter`,`lightnessJitter`,`darknessJitter`,`jitterPerCopy`,`positionJitterLateral`,`positionJitterLinear`];for(let e of Fr)e!==`brushTool`&&(B(e).addEventListener(`input`,cr),B(e).addEventListener(`change`,cr));B(`brushTool`).addEventListener(`change`,()=>{ir(B(`brushTool`).value===`blend`?`blend`:`paint`,!0),cr(),Z()}),Vn.addEventListener(`input`,sr),Tn.addEventListener(`click`,()=>{Tn.disabled||ar(!er)}),Fn.addEventListener(`click`,()=>{jr()}),In.addEventListener(`click`,()=>{Ar(`undo`)}),Ln.addEventListener(`click`,()=>{Ar(`redo`)}),Rn.addEventListener(`click`,()=>{!X()&&Q===null&&K.fitView()}),zn.addEventListener(`click`,()=>{!X()&&Q===null&&K.zoomBy(1.35)}),Bn.addEventListener(`click`,()=>{!X()&&Q===null&&K.zoomBy(1/1.35)}),Pn.addEventListener(`change`,async()=>{if(X()||Q!==null){Pn.value=K.getStats().layerFormat;return}let e=Pn.value;Zn=!0,Pn.disabled=!0,Z(),Y();try{await K.setLayerFormat(e)||(Pn.value=K.getStats().layerFormat)}catch{Pn.value=K.getStats().layerFormat}finally{Zn=!1,G=K.getHistoryState(),Z(),Y()}}),Dn.addEventListener(`click`,async()=>{if(!X()){ar(!1),Xn=!0,Dn.disabled=!0,Z(),Y(),On.textContent=`Benchmark in esecuzione sulla GPU…`;try{let e=await K.runBenchmark(V(`benchmarkStamps`));On.textContent=[`${H(e.baseStamps)} base stamps`,`${H(e.logicalCopies)} copie logiche`,`CPU submit ${e.cpuSubmitMs.toFixed(2)} ms`,`GPU completion ${e.gpuCompletionMs.toFixed(2)} ms`,`copertura teorica ${(e.estimatedCoveredFragments/1e6).toFixed(1)} Mpx`,e.strategy].join(` · `)}catch(e){On.textContent=e instanceof Error?e.message:String(e)}finally{Xn=!1,Dn.disabled=!1,G=K.getHistoryState(),Z(),Y()}}}),kn.addEventListener(`click`,()=>{X()||Jn||Yn||qn||Gn||q||(Kn=!Kn,Kn?(Cr(),ar(!1),W.textContent=`Preset umano applicato. Disegna ora una sola pennellata sul canvas.`):W.textContent=q?Mr(q):`Registrazione annullata.`,Y())}),An.addEventListener(`click`,()=>{Br()});function Ir(e){B(`fpsStat`).textContent=`${e.fps}`,B(`cpuStat`).textContent=`${e.lastCpuFrameMs.toFixed(2)} ms`,B(`stampStat`).textContent=H(e.totalBaseStamps),B(`avoidedStat`).textContent=H(e.avoidedLogicalDraws),B(`memoryStat`).textContent=`${e.layerMemoryMiB} MiB`,B(`gpuStat`).textContent=e.gpuLabel}function Lr(e,t){let n=Cr(),r=K.toLayerPoint(t);Gn={settings:n,startTimestamp:e.timeStamp,points:[{...r,timeMs:0}]},W.textContent=`Registrazione in corso…`}function Rr(e,t){let n=Gn;if(n)for(let r=0;r<t.length;r+=1){let i=n.points[n.points.length-1]?.timeMs??0,a=Math.max(i,e[r].timeStamp-n.startTimestamp,0);n.points.push({...K.toLayerPoint(t[r]),timeMs:a})}}async function zr(e){let t=Gn;if(Gn=null,Kn=!1,t&&e&&t.points.length>1){let e={version:1,capturedAt:new Date().toISOString(),settings:t.settings,points:t.points};Yn=!0,W.textContent=`Fissaggio permanente del tratto di riferimento…`,Y();try{q=await br(e),vr(),W.textContent=Mr(q)}catch(e){W.textContent=e instanceof Error?e.message:String(e)}finally{Yn=!1}}else t&&(W.textContent=`Tratto troppo breve: registra una pennellata con almeno un movimento.`);Y()}async function Br(){let e=q;if(!e||qn||X())return;let t=wr(),n=Tr(),r=Er(),i=Dr(e,t,n,r),a=Or(t,n,r);ar(!1),qn=!0,Dn.disabled=!0,Y(),Z(),Sr(i),W.textContent=`Riproduzione test ${a} in corso…`;try{if(await K.waitForIdle(),K.resetStrokeRandomSeed(),!K.resetDocument())throw Error(`Il documento è occupato da un'operazione Undo/Redo.`);await K.waitForIdle();let o=K.getStats(),s=performance.now(),c=e.points[e.points.length-1],l=[],u=[],d=1;K.startStrokePerformanceProfile();let f=performance.now();K.beginStrokeAtLayer(e.points[0]),u.push(performance.now()-f),await new Promise(t=>{let n=r=>{let i=r-s,a=[];for(;d<e.points.length&&e.points[d].timeMs<=i;)l.push(Math.max(0,i-e.points[d].timeMs)),a.push(e.points[d]),d+=1;if(a.length>0){let e=performance.now();K.extendStrokeAtLayer(a),u.push(performance.now()-e)}if(d<e.points.length){requestAnimationFrame(n);return}K.endStroke(c.timeMs),t()};requestAnimationFrame(n)});let p=performance.now();await K.waitForIdle();let m=performance.now();await dr();let h=performance.now(),g=K.finishStrokePerformanceProfile();if(!g)throw Error(`Profilo del tratto non disponibile.`);let _=K.getStats(),v=Math.max(0,_.totalBaseStamps-o.totalBaseStamps),y=v*i.count,b={inputDeliveryMs:p-s,inputDelayP50Ms:ur(l,.5),inputDelayP95Ms:ur(l,.95),inputDelayMaxMs:l.length===0?0:Math.max(...l),layerInputDispatchTotalMs:u.reduce((e,t)=>e+t,0),layerInputDispatchP50Ms:ur(u,.5),layerInputDispatchP95Ms:ur(u,.95),layerInputDispatchMaxMs:u.length===0?0:Math.max(...u),inputDeliveryPath:`preconverted-layer-points`,pointerPipelineMeasured:!1,inputToGpuCompletionMs:Math.max(0,m-p),endToPresentedMs:Math.max(0,h-s)},x=await hr({version:1,recordedAt:new Date().toISOString(),benchmark:{capturedAt:e.capturedAt,traceFingerprint:fr(e.points),pointCount:e.points.length,traceDurationMs:c.timeMs,...pr(e.points),testVariant:t,testBlendMode:n,testGrainMode:r,settings:i},playback:b,performance:g,environment:mr()});W.textContent=[`Test ${a}`,`Tratto ${lr(c.timeMs)}`,`${H(e.points.length)} campioni`,`${H(v)} stamps base`,`${H(y)} copie fisiche`,`coda GPU ${b.inputToGpuCompletionMs.toFixed(2)} ms`,`CPU frame p95 ${g.renderFrameTotalP95Ms.toFixed(2)} ms`,`submit p95 ${g.submitImmediateP95Ms.toFixed(2)} ms`,`display mip ${g.paintDisplaySelectedMipLevel} / ${H(g.paintDisplayPyramidPasses)} pass`,g.adaptivePreviewActivations>0?`preview tip ${H(g.adaptivePreviewBaseStampsDrawn)} stamp / ${g.adaptivePreviewJsTotalMs.toFixed(2)} ms JS`:`preview tip non attivata`,`spacing adattivo ${g.adaptiveSpacingInitialPercent.toFixed(2)}→${g.adaptiveSpacingFinalPercent.toFixed(2)}% / ${g.adaptiveSpacingIncreaseCount} step`,`history CPU ${H(g.historyCapturedBaseStamps)} stamp / ${H(g.historyCapturedBatches)} batch`,`FPS medi ${g.averageRenderFps.toFixed(1)}`,`${H(g.delayedRenderFrames)} frame >20 ms`,`presentazione ${b.endToPresentedMs.toFixed(2)} ms`,x>0?`run #${x} salvata`:`run salvata`].join(` · `)}catch(e){K.finishStrokePerformanceProfile(),W.textContent=e instanceof Error?e.message:String(e)}finally{qn=!1,Dn.disabled=!1,Y(),G=K.getHistoryState(),Z()}}function Vr(e){return e.pointerType===`mouse`?1:e.pressure>0?e.pressure:e.pointerType===`pen`?.5:.65}function Hr(e){return{clientX:e.clientX,clientY:e.clientY,pressure:Vr(e),timeMs:e.timeStamp}}var Q=null,$=null,Ur=0,Wr=0,Gr=new Map,Kr=null;function qr(){let e=[...Gr.values()];if(e.length<2)return null;let t=e[0],n=e[1];return{centerX:(t.clientX+n.clientX)*.5,centerY:(t.clientY+n.clientY)*.5,distance:Math.max(1,Math.hypot(n.clientX-t.clientX,n.clientY-t.clientY))}}function Jr(){Gn&&(Gn=null,Kn=!1,W.textContent=`Registrazione annullata dal gesto a due dita.`,Y())}function Yr(){$!==`touch-navigation`&&($===`paint`&&(K.cancelStrokeBeforeRender()||K.endStroke(),Jr()),$=`touch-navigation`,U.classList.add(`panning`)),Kr=qr()}U.addEventListener(`pointerdown`,e=>{if(e.pointerType===`touch`&&Q!==null&&Gr.size>0&&!kr()){e.preventDefault(),Gr.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),U.setPointerCapture(e.pointerId),Gr.size>=2&&Yr();return}if(!(Q!==null||kr())){if(e.preventDefault(),Q=e.pointerId,e.pointerType===`touch`&&Gr.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),$=e.shiftKey||e.button===1||e.button===2?`pan`:`paint`,U.setPointerCapture(e.pointerId),$===`pan`)U.classList.add(`panning`),Ur=e.clientX,Wr=e.clientY;else{let t=Hr(e);Kn&&Lr(e,t),K.beginStroke(t)}requestAnimationFrame(()=>{Q===e.pointerId&&(Z(),Y())})}}),U.addEventListener(`pointermove`,e=>{if(e.pointerType===`touch`&&Gr.has(e.pointerId)&&(Gr.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),$===`touch-navigation`)){e.preventDefault();let t=qr(),n=Kr;if(t&&n){let e=t.centerX-n.centerX,r=t.centerY-n.centerY;(Math.abs(e)>.01||Math.abs(r)>.01)&&K.panByClientDelta(e,r);let i=t.distance/n.distance;Number.isFinite(i)&&Math.abs(i-1)>1e-4&&K.zoomBy(Math.min(2,Math.max(.5,i)),t.centerX,t.centerY)}Kr=t;return}if(e.pointerId!==Q||$===null)return;if(e.preventDefault(),$===`pan`){K.panByClientDelta(e.clientX-Ur,e.clientY-Wr),Ur=e.clientX,Wr=e.clientY;return}let t=e.getCoalescedEvents?.()??[],n=t.length>0?t:[e],r=n.map(Hr);Rr(n,r),K.extendStroke(r)});function Xr(e){if(e.pointerType===`touch`&&Gr.delete(e.pointerId),$===`touch-navigation`){e.preventDefault();let t=Gr.keys().next().value;Q=typeof t==`number`?t:null,Kr=qr(),Gr.size===0&&(U.classList.remove(`panning`),$=null,G=K.getHistoryState(),Z(),Y());return}e.pointerId===Q&&($===`paint`&&(K.endStroke(e.timeStamp),zr(e.type===`pointerup`)),U.classList.remove(`panning`),$=null,Q=null,Kr=null,G=K.getHistoryState(),Z(),Y())}U.addEventListener(`pointerup`,Xr),U.addEventListener(`pointercancel`,Xr),U.addEventListener(`lostpointercapture`,Xr),U.addEventListener(`contextmenu`,e=>e.preventDefault()),window.addEventListener(`keydown`,e=>{if(e.defaultPrevented||e.repeat||e.isComposing||e.altKey||!e.ctrlKey&&!e.metaKey||e.key.toLowerCase()!==`z`||(e.target instanceof Element?e.target:null)?.closest(`input, textarea, select, [contenteditable]`))return;let t=e.shiftKey?`redo`:`undo`;!(t===`undo`?G.canUndo:G.canRedo)||X()||Q!==null||(e.preventDefault(),Ar(t))}),U.addEventListener(`wheel`,e=>{if(e.preventDefault(),X()||Q!==null)return;let t=Math.exp(-e.deltaY*.0015);K.zoomBy(Math.min(2,Math.max(.5,t)),e.clientX,e.clientY)},{passive:!1}),new ResizeObserver(()=>K.resizeCanvas()).observe(U),ar(!0),ir(`paint`,!1),sr(),K.setBrushSettings(or()),Y(),Z(),xr(),K.initialize().then(()=>{$n=!0,G=K.getHistoryState(),Z(),Y()}).catch(e=>{let t=window.isSecureContext?``:` WebGPU richiede HTTPS oppure localhost; un indirizzo LAN in HTTP non è sufficiente.`;En.textContent=`${e instanceof Error?e.message:String(e)}${t}`,En.className=`status error`,Dn.disabled=!0,Z()}),window.setInterval(()=>Ir(K.getStats()),500);