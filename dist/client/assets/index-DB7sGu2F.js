...
`,oe=192,se=256,I=4096,ce=`dry-blend-webgpu-v3-compute-fused-sweep`;function le(e,t,n){return Math.min(n,Math.max(t,e))}function ue(e,t){if(!e)return{...t};let n=Math.min(e.x,t.x),r=Math.min(e.y,t.y),i=Math.max(e.x+e.width,t.x+t.width),a=Math.max(e.y+e.height,t.y+t.height);return{x:n,y:r,width:i-n,height:a-r}}function de(e){let t=le(e,0,1);return t<=.04045?t/12.92:((t+.055)/1.055)**2.4}function fe(e){let t=e.trim().replace(/^#/,``);if(!/^[0-9a-fA-F]{6}$/.test(t))throw Error(`Colore HEX Blend non valido: ${e}`);return[de(Number.parseInt(t.slice(0,2),16)/255),de(Number.parseInt(t.slice(2,4),16)/255),de(Number.parseInt(t.slice(4,6),16)/255)]}function pe(e){return{x:e.x,y:e.y,width:e.width,height:e.height}}function me(e){return{build:e.build,stepCount:1,steps:[{...e.steps[0]}],empty:e.empty,readRect:pe(e.readRect),writeRect:pe(e.writeRect)}}async function he(e){let t=(await Promise.all(e.map(async({label:e,module:t})=>({label:e,messages:(await t.getCompilationInfo()).messages})))).flatMap(({label:e,messages:t})=>[...t].filter(e=>e.type===`error`).map(t=>`${e}:${t.lineNum}:${t.linePos} ${t.message}`));if(t.length>0)throw Error(`Shader Blend WGSL non valido:\n${t.join(`
`)}`)}var ge=class e{static async create(t){let n=new e(t);try{return await n.initialize(),n}catch(e){throw n.destroy(),e}}build=ce;maximumBatchesPerSubmit=se;device;documentWidth;documentHeight;layerFormat;layerView;layerSamplingView;shapeMaskView;shapeMaskSampler;grainTextureView;grainSamplers;scratchSize;uniformStride;uniformUpload;uniformBuffer;gatherBindGroupLayout;pickupBindGroupLayout;depositBindGroupLayout;scatterBindGroupLayout;gatherPipeline;pickupPipeline;depositPipeline;scatterPipeline;scratch=null;activeHistoryActionId=null;carrierCursor=0;carrierValid=!1;destroyed=!1;constructor(e){this.device=e.device,this.documentWidth=e.documentWidth,this.documentHeight=e.documentHeight,this.layerFormat=e.layerFormat,this.layerView=e.layerView,this.layerSamplingView=e.layerSamplingView,this.shapeMaskView=e.shapeMaskView,this.shapeMaskSampler=e.shapeMaskSampler,this.grainTextureView=e.grainTextureView,this.grainSamplers=e.grainSamplers,this.scratchSize=e.scratchSize??1664,this.uniformStride=Math.ceil(oe/this.device.limits.minUniformBufferOffsetAlignment)*this.device.limits.minUniformBufferOffsetAlignment,this.uniformUpload=new ArrayBuffer(this.uniformStride*se),this.uniformBuffer=this.device.createBuffer({label:`Blend dry dynamic uniforms`,size:this.uniformUpload.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST})}async initialize(){let e=e=>({binding:0,visibility:e,buffer:{type:`uniform`,hasDynamicOffset:!0,minBindingSize:oe}}),t=(e,t,n)=>({binding:e,visibility:t,buffer:{type:n?`read-only-storage`:`storage`}}),n=e=>({binding:e,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:`float`,viewDimension:`2d`,multisampled:!1}}),r=e=>({binding:e,visibility:GPUShaderStage.COMPUTE,sampler:{type:`filtering`}});this.gatherBindGroupLayout=this.device.createBindGroupLayout({label:`Blend dry gather bind group layout`,entries:[e(GPUShaderStage.COMPUTE),n(1),t(2,GPUShaderStage.COMPUTE,!1),t(3,GPUShaderStage.COMPUTE,!1)]}),this.pickupBindGroupLayout=this.device.createBindGroupLayout({label:`Blend dry pickup bind group layout`,entries:[e(GPUShaderStage.COMPUTE),t(1,GPUShaderStage.COMPUTE,!0),t(2,GPUShaderStage.COMPUTE,!1)]}),this.depositBindGroupLayout=this.device.createBindGroupLayout({label:`Blend dry deposit bind group layout`,entries:[e(GPUShaderStage.COMPUTE),t(1,GPUShaderStage.COMPUTE,!1),t(2,GPUShaderStage.COMPUTE,!1),t(3,GPUShaderStage.COMPUTE,!0),n(4),r(5),n(6),r(7)]}),this.scatterBindGroupLayout=this.device.createBindGroupLayout({label:`Blend dry scatter bind group layout`,entries:[e(GPUShaderStage.FRAGMENT),t(1,GPUShaderStage.FRAGMENT,!0),t(2,GPUShaderStage.FRAGMENT,!0)]});let i=[{label:`Blend gather`,module:this.device.createShaderModule({label:`Blend gather WGSL`,code:re})},{label:`Blend pickup`,module:this.device.createShaderModule({label:`Blend pickup WGSL`,code:F})},{label:`Blend deposit`,module:this.device.createShaderModule({label:`Blend deposit WGSL`,code:ie})},{label:`Blend scatter`,module:this.device.createShaderModule({label:`Blend scatter WGSL`,code:ae})}];await he(i);let a=(e,t)=>this.device.createPipelineLayout({label:e,bindGroupLayouts:[t]}),o=(e,t,n,r)=>this.device.createComputePipeline({label:e,layout:a(`${e} pipeline layout`,t),compute:{module:n,entryPoint:r}});this.device.pushErrorScope(`validation`),this.gatherPipeline=o(`Blend dry gather ROI`,this.gatherBindGroupLayout,i[0].module,`gatherMain`),this.pickupPipeline=o(`Blend dry 8x8 weighted pigment pickup`,this.pickupBindGroupLayout,i[1].module,`pickupMain`),this.depositPipeline=o(`Blend dry fused sweep mask and pigment deposit`,this.depositBindGroupLayout,i[2].module,`depositMain`),this.scatterPipeline=this.device.createRenderPipeline({label:`Blend dry scatter to canonical layer`,layout:a(`Blend dry scatter pipeline layout`,this.scatterBindGroupLayout),vertex:{module:i[3].module,entryPoint:`fullscreenVertex`},fragment:{module:i[3].module,entryPoint:`scatterFragment`,targets:[{format:this.layerFormat}]},primitive:{topology:`triangle-list`}});let s=await this.device.popErrorScope();if(s)throw Error(`Pipeline Blend WebGPU non valida: ${s.message}`)}beginStroke(e){this.assertAlive(),this.activeHistoryActionId=e,this.carrierCursor=0,this.carrierValid=!1}submit(e,t,n,r){if(this.assertAlive(),e.length>se)throw RangeError(`Blend dry accetta al massimo ${se} batch per submit.`);this.activeHistoryActionId!==n&&this.beginStroke(n);let i=performance.now(),a=this.ensureScratchResources(),o=e.filter(e=>!e.empty);for(let e of o)this.validateBatch(e);let s=this.buildStepGroups(o);this.populateUniforms(o,s,t),o.length>0&&this.device.queue.writeBuffer(this.uniformBuffer,0,this.uniformUpload,0,o.length*this.uniformStride);let c=this.device.createCommandEncoder({label:`Blend dry frame encoder`}),l=0,u=null;r&&(c.beginRenderPass({label:`Blend dry clear canonical layer`,colorAttachments:[{view:this.layerView,loadOp:`clear`,storeOp:`store`,clearValue:{r:0,g:0,b:0,a:0}}]}).end(),l+=1);let d=e=>Math.ceil(e/8),f=t.grainMode===`moving`?`moving`:`fixed`;for(let e of s){let n=e.start*this.uniformStride,r=c.beginComputePass({label:`Blend dry gather + fused sweep steps`});r.setPipeline(this.gatherPipeline),r.setBindGroup(0,a.gatherBindGroup,[n]),r.dispatchWorkgroups(d(e.readRect.width),d(e.readRect.height));for(let n=0;n<e.count;n+=1){let i=o[e.start+n],s=(e.start+n)*this.uniformStride;r.setPipeline(this.pickupPipeline),r.setBindGroup(0,a.pickupBindGroup,[s]),r.dispatchWorkgroups(1),r.setPipeline(this.depositPipeline),r.setBindGroup(0,a.depositBindGroups[f][t.grainFiltering],[s]),r.dispatchWorkgroups(d(i.writeRect.width),d(i.writeRect.height))}r.end();let i=c.beginRenderPass({label:`Blend dry scatter ROI to canonical layer`,colorAttachments:[{view:this.layerView,loadOp:`load`,storeOp:`store`}]});i.setPipeline(this.scatterPipeline),i.setBindGroup(0,a.scatterBindGroup,[n]),i.setScissorRect(e.writeRect.x,e.writeRect.y,e.writeRect.width,e.writeRect.height),i.draw(3),i.end(),l+=2,u=ue(u,e.writeRect)}return(r||o.length>0)&&this.device.queue.submit([c.finish()]),o.length>0&&(this.carrierValid=!0),{dirtyRect:u,batchCount:o.length,passCount:l,scratchAllocated:this.scratch!==null,cpuMs:performance.now()-i}}memoryMiB(){if(!this.scratch)return 0;let e=this.scratchSize*this.scratchSize,t=e*16,n=e*4,r=I*16;return(t+n+r+this.uniformUpload.byteLength)/(1024*1024)}allocatedMemoryMiB(){return this.scratch?this.memoryMiB():this.uniformUpload.byteLength/(1024*1024)}prewarmScratch(){this.assertAlive();let e=this.scratch!==null;return this.ensureScratchResources(),!e}releaseScratch(){return this.destroyed||!this.scratch?!1:(this.scratch.stateBuffer.destroy(),this.scratch.coverageBuffer.destroy(),this.scratch.carrierBuffer.destroy(),this.scratch=null,this.carrierValid=!1,!0)}destroy(){this.destroyed||(this.destroyed=!0,this.uniformBuffer.destroy(),this.scratch&&=(this.scratch.stateBuffer.destroy(),this.scratch.coverageBuffer.destroy(),this.scratch.carrierBuffer.destroy(),null))}assertAlive(){if(this.destroyed)throw Error(`Renderer Blend dry già distrutto.`)}validateBatch(e){if(e.build!==`dry-blend-continuous-core-v1-pressure-inert`||e.stepCount!==1)throw Error(`Batch Blend dry incompatibile.`);if(![e.readRect.x,e.readRect.y,e.readRect.width,e.readRect.height,e.writeRect.x,e.writeRect.y,e.writeRect.width,e.writeRect.height].every(Number.isFinite))throw TypeError(`Rettangolo Blend dry non valido.`);if(e.readRect.width<=0||e.readRect.height<=0||e.readRect.width>this.scratchSize||e.readRect.height>this.scratchSize)throw RangeError(`ROI Blend dry oltre lo scratch WebGPU.`);if(e.writeRect.x<0||e.writeRect.y<0||e.writeRect.width<=0||e.writeRect.height<=0||e.writeRect.x+e.writeRect.width>this.documentWidth||e.writeRect.y+e.writeRect.height>this.documentHeight)throw RangeError(`Dirty rect Blend dry fuori dal layer.`)}buildStepGroups(e){let t=[],n=null;for(let r=0;r<e.length;r+=1){let i=e[r];if(n){let e=ue(n.readRect,i.readRect);if(e.width<=this.scratchSize&&e.height<=this.scratchSize){n.count+=1,n.readRect=e,n.writeRect=ue(n.writeRect,i.writeRect);continue}t.push(n)}n={start:r,count:1,readRect:pe(i.readRect),writeRect:pe(i.writeRect)}}return n&&t.push(n),t}populateUniforms(e,t,n){let r=fe(n.color),i=n.grainInvert?-1:1,a=le(n.grainScale,.1,4),o=n.grainMode===`moving`?2:+(n.grainMode===`texturized`),s=n.grainFiltering===`no`?0:n.grainFiltering===`classic`?1:2;for(let c of t)for(let t=0;t<c.count;t+=1){let l=c.start+t,u=e[l],d=u.steps[0],f=this.carrierCursor,p=(this.carrierCursor+1)%I;this.carrierCursor=p;let m=l*this.uniformStride,h=new Float32Array(this.uniformUpload,m,oe/4),g=new Uint32Array(this.uniformUpload,m,oe/4);h.fill(0),h[0]=this.documentWidth,h[1]=this.documentHeight,h[2]=c.readRect.x,h[3]=c.readRect.y,h[4]=c.readRect.width,h[5]=c.readRect.height,h[6]=d.fromX,h[7]=d.fromY,h[8]=d.toX,h[9]=d.toY,h[10]=d.fromHalfWidth,h[11]=d.fromHalfHeight,h[12]=d.toHalfWidth,h[13]=d.toHalfHeight,h[14]=d.fromAngle,h[15]=d.toAngle,h[16]=le(n.hardness,0,1),h[17]=le(d.flow,0,1),h[18]=d.spacing,h[19]=d.arcStart,h[20]=d.distance,h[21]=d.diameter,h[22]=le(d.warpStrength,0,1),h[23]=S(n.blendStretch),h[24]=1/(2500*a),h[25]=C(n.blendPaint),h[26]=le(n.grainDepth,0,1),h[27]=le(n.grainBrightness,-1,1)*i,h[28]=(1+le(n.grainContrast,-1,1))*i,h[31]=0,h[32]=r[0],h[33]=r[1],h[34]=r[2],h[35]=1,h[36]=u.writeRect.x-c.readRect.x,h[37]=u.writeRect.y-c.readRect.y,h[38]=u.writeRect.width,h[39]=u.writeRect.height,g[40]=+(n.shape===`shape`),g[41]=o,g[42]=s,g[43]=this.carrierValid||l>0?1:0,g[44]=f,g[45]=p,g[46]=this.scratchSize,g[47]=0}}createDepositBindGroups(e,t,n){let r={binding:0,resource:{buffer:this.uniformBuffer,offset:0,size:oe}},i={fixed:{},moving:{}};for(let a of[`fixed`,`moving`])for(let o of[`no`,`classic`,`improved`])i[a][o]=this.device.createBindGroup({label:`Blend dry deposit ${a} ${o}`,layout:this.depositBindGroupLayout,entries:[r,{binding:1,resource:{buffer:e}},{binding:2,resource:{buffer:t}},{binding:3,resource:{buffer:n}},{binding:4,resource:this.shapeMaskView},{binding:5,resource:this.shapeMaskSampler},{binding:6,resource:this.grainTextureView},{binding:7,resource:this.grainSamplers[a][o]}]});return i}setGrainTextureView(e){this.destroyed||this.grainTextureView===e||(this.grainTextureView=e,this.rebuildResidentDepositBindGroups())}setShapeMaskView(e){this.destroyed||this.shapeMaskView===e||(this.shapeMaskView=e,this.rebuildResidentDepositBindGroups())}rebuildResidentDepositBindGroups(){this.scratch&&(this.scratch.depositBindGroups=this.createDepositBindGroups(this.scratch.stateBuffer,this.scratch.coverageBuffer,this.scratch.carrierBuffer))}ensureScratchResources(){if(this.scratch)return this.scratch;let e=this.scratchSize*this.scratchSize,t=this.device.createBuffer({label:`Blend dry scratch state`,size:e*16,usage:GPUBufferUsage.STORAGE}),n=this.device.createBuffer({label:`Blend dry union coverage`,size:e*4,usage:GPUBufferUsage.STORAGE}),r=this.device.createBuffer({label:`Blend dry carrier ring`,size:I*16,usage:GPUBufferUsage.STORAGE}),i={binding:0,resource:{buffer:this.uniformBuffer,offset:0,size:oe}},a=(e,t)=>({binding:e,resource:{buffer:t}}),o=this.device.createBindGroup({label:`Blend dry gather bind group`,layout:this.gatherBindGroupLayout,entries:[i,{binding:1,resource:this.layerSamplingView},a(2,t),a(3,n)]}),s=this.device.createBindGroup({label:`Blend dry pickup bind group`,layout:this.pickupBindGroupLayout,entries:[i,a(1,t),a(2,r)]}),c=this.createDepositBindGroups(t,n,r),l=this.device.createBindGroup({label:`Blend dry scatter bind group`,layout:this.scatterBindGroupLayout,entries:[i,a(1,t),a(2,n)]});return this.scratch={stateBuffer:t,coverageBuffer:n,carrierBuffer:r,gatherBindGroup:o,pickupBindGroup:s,depositBindGroups:c,scatterBindGroup:l},this.scratch}},_e=`
...
`,Ee=`time-window-quadratic-ease-out-start-end-tail-holdback`;function De(e,t,n){return Math.min(n,Math.max(t,e))}function Oe(e){let t=1-De(e,0,1);return 1-t*t}function ke(e,t){let n=De(e,0,2),r=Oe(t/100);return n+(1-n)*r}function Ae(e,t,n){let r=De(e,0,2),i=De(t,0,2),a=Oe(n/100);return i+(r-i)*a}function je(e,t,n,r){return Math.max(0,e)*Ae(t,n,r)}function Me(e){return Math.abs(De(e,0,2)-1)>2**-52*8}function Ne(e,t){return Math.abs(De(e,0,2)-1)<=2**-52*8&&!Me(t)}var Pe=`width-tiered-1024-through-128-otherwise-2048`,Fe=1024,Ie=2048;function Le(e){return Math.min(512,Math.max(0,Number(e)||0))<=128?Fe:Ie}var Re=[`inside`,`center`,`outside`],ze=Object.freeze([1,.643,.282,1]),Be=Object.freeze({enabled:!1,width:14,position:`outside`,color:ze}),Ve=(e,t,n)=>Math.max(t,Math.min(n,e));function He(e){return e&&typeof e==`object`?e:{}}function Ue(e){return e&&typeof e.length==`number`?e:null}function We(e={}){let t=He(e),n=Number(t.width??Be.width),r=Number.isFinite(n)?Ve(n,0,512):Be.width,i=String(t.position??Be.position).toLowerCase(),a=Re.includes(i)?i:Be.position,o=Ue(t.color)??ze,s=Array.from({length:4},(e,t)=>{let n=ze[t],r=Number(o[t]??n);return Number.isFinite(r)?Ve(r,0,1):n});return{enabled:!!t.enabled,width:r,position:a,color:s}}function Ge(e={}){let t=We(e);return{...t,color:[...t.color]}}function Ke(e,t){let n=We(e),r=We(t);return n.enabled===r.enabled&&n.width===r.width&&n.position===r.position&&n.color.every((e,t)=>e===r.color[t])}var qe=(e,t=1)=>{let n=Math.trunc(Number(e));return Number.isFinite(n)?n:t};function Je(e,{plusOne:t=!0}={}){let n=Math.max(1,qe(e)),r=1;for(;r<n;)r*=2;r=Math.max(1,r>>1);let i=[];for(;r>=1;r>>=1)i.push(r);return t&&i.push(1),i}var Ye=`raster-stroke-webgpu-v5-direct-lod0-coarse-styled-mips-packed-r8-coverage-native-unorm-round-even`,Xe=`persistent-packed-r8-style-coverage`,Ze=`register-only-during-coverage-resolve`,Qe=`threshold-change-or-existing-coverage-one-pixel-halo`,$e=`direct-lod0-plus-derived-mips-1-through-12`,et=2048,L=8,tt=80,nt=tt,rt=256,it=2048,at=4294967295,ot=32,st=4,ct=1,lt=3,ut=lt*4,dt=64;function ft(e,t,n){return Math.max(t,Math.min(n,e))}function pt(e){return e===`light-glaze`?1:e===`thickness-tail`?2:0}function mt(e){return e===`inside`?0:e===`center`?1:2}function ht(e,t,n){if(!e)return null;let r=ft(Math.floor(e.x),0,t),i=ft(Math.floor(e.y),0,n),a=ft(Math.ceil(e.x+e.width),0,t),o=ft(Math.ceil(e.y+e.height),0,n);return a>r&&o>i?{x:r,y:i,width:a-r,height:o-i}:null}function gt(e,t,n){let r=ht(e,t,n);if(!r)return null;let i=Math.floor(r.x/st)*st,a=Math.min(t,Math.ceil((r.x+r.width)/st)*st);return{x:i,y:r.y,width:a-i,height:r.height}}function _t(e,t,n){let r=ht(e,t,n);return r?ht({x:r.x-ct,y:r.y-ct,width:r.width+ct*2,height:r.height+ct*2},t,n):null}function vt(e,t,n){let r=ht(e,t*2,n*2);if(!r)return null;let i=Math.floor(r.x/2),a=Math.floor(r.y/2),o=Math.min(t,Math.ceil((r.x+r.width)/2)),s=Math.min(n,Math.ceil((r.y+r.height)/2));return o>i&&s>a?{x:i,y:a,width:o-i,height:s-a}:null}function yt(e,t,n=0){return`
...
`}function wt(e,t,n){return`${yt(e,t)}
${Ct(e)}
...
`}function Tt(e,t,n){return`${yt(e,t)}
${Ct(e)}
...
`}function Dt(e,t){let n=Math.ceil(e/ot);return`${yt(e,t)}
...
const THRESHOLD_WORD_BITS = ${ot}u;
