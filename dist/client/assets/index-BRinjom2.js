...
`,De=`time-window-quadratic-ease-out-start-end-tail-holdback`;function Oe(e,t,n){return Math.min(n,Math.max(t,e))}function ke(e){let t=1-Oe(e,0,1);return 1-t*t}function Ae(e,t){let n=Oe(e,0,2),r=ke(t/100);return n+(1-n)*r}function je(e,t,n){let r=Oe(e,0,2),i=Oe(t,0,2),a=ke(n/100);return i+(r-i)*a}function Me(e,t,n,r){return Math.max(0,e)*je(t,n,r)}function Ne(e){return Math.abs(Oe(e,0,2)-1)>2**-52*8}function Pe(e,t){return Math.abs(Oe(e,0,2)-1)<=2**-52*8&&!Ne(t)}var Fe=[`inside`,`center`,`outside`],Ie=Object.freeze([1,.643,.282,1]),Le=Object.freeze({enabled:!1,width:14,position:`outside`,color:Ie}),Re=(e,t,n)=>Math.max(t,Math.min(n,e));function ze(e){return e&&typeof e==`object`?e:{}}function Be(e){return e&&typeof e.length==`number`?e:null}function Ve(e={}){let t=ze(e),n=Number(t.width??Le.width),r=Number.isFinite(n)?Re(n,0,512):Le.width,i=String(t.position??Le.position).toLowerCase(),a=Fe.includes(i)?i:Le.position,o=Be(t.color)??Ie,s=Array.from({length:4},(e,t)=>{let n=Ie[t],r=Number(o[t]??n);return Number.isFinite(r)?Re(r,0,1):n});return{enabled:!!t.enabled,width:r,position:a,color:s}}function He(e={}){let t=Ve(e);return{...t,color:[...t.color]}}function Ue(e,t){let n=Ve(e),r=Ve(t);return n.enabled===r.enabled&&n.width===r.width&&n.position===r.position&&n.color.every((e,t)=>e===r.color[t])}function We(e,t,n){return n?t:Math.max(e,t)}var Ge=(e,t=1)=>{let n=Math.trunc(Number(e));return Number.isFinite(n)?n:t};function Ke(e,{plusOne:t=!0}={}){let n=Math.max(1,Ge(e)),r=1;for(;r<n;)r*=2;r=Math.max(1,r>>1);let i=[];for(;r>=1;r>>=1)i.push(r);return t&&i.push(1),i}var qe=`raster-stroke-webgpu-v2-compute-threshold-gated-packed-dual-jfa-q10.6`,Je=2048,F=8,Ye=80,Xe=256,Ze=2048,Qe=4294967295,$e=32,et=3,tt=et*4,nt=64;function rt(e,t,n){return Math.max(t,Math.min(n,e))}function it(e){return e===`light-glaze`?1:e===`thickness-tail`?2:0}function at(e){return e===`inside`?0:e===`center`?1:2}function ot(e,t,n){if(!e)return null;let r=rt(Math.floor(e.x),0,t),i=rt(Math.floor(e.y),0,n),a=rt(Math.ceil(e.x+e.width),0,t),o=rt(Math.ceil(e.y+e.height),0,n);return a>r&&o>i?{x:r,y:i,width:a-r,height:o-i}:null}function st(e,t,n){let r=ot(e,t,n);if(!r)return null;let i=Math.floor(r.x/2)*2,a=Math.min(t,Math.ceil((r.x+r.width)/2)*2);return{x:i,y:r.y,width:a-i,height:r.height}}function ct(e,t){return`
...
`}function lt(e,t,n){return`${ct(e,t)}
...
fn main(@builtin(global_invocation_id) globalId: vec3<u32>) {
  ...
  outputSeeds[localPosition.y * ${n}u + localPosition.x] = value;
}
...
fn bestCandidate(position: vec2<u32>, candidateIndex: u32) -> u32 {
  ...
  for (var offsetY = -1; offsetY <= 1; offsetY += 1) {
    for (var offsetX = -1; offsetX <= 1; offsetX += 1) {
      ...
      let pair = inputSeeds[
        u32(samplePosition.y) * ${e}u + u32(samplePosition.x)
      ];
...
fn main(@builtin(global_invocation_id) globalId: vec3<u32>) {
  ...
  outputSeeds[position.y * ${e}u + position.x] = vec2<u32>(
    bestCandidate(position, 0u),
    bestCandidate(position, 1u)
  );
}
`}function dt(e,t,n){return`${ct(e,t)}
...
fn resolveFixedDistance(
  ...
  let pair = propagatedSeeds[
    localPosition.y * ${n}u + localPosition.x
  ];
...
`}function ft(e,t,n){return`${ct(e,t)}
...
`}function pt(e,t){let n=Math.ceil(e/$e);return`${ct(e,t)}
