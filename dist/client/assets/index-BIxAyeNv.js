...
`);throw Error(`Errore WGSL nel modulo ${t}:\n${r}`)}describeAdapter(e){let t=e.info,n=[t.vendor,t.architecture,t.device,t.description].map(e=>e?.trim()).filter(e=>!!e);return[...new Set(n)].join(` · `)||`GPU WebGPU`}};function d(e){let t=document.getElementById(e);if(!t)throw Error(`Elemento #${e} non trovato.`);return t}function f(e){return Number(d(e).value)}function p(e){return new Intl.NumberFormat(`it-IT`,{maximumFractionDigits:0}).format(e)}var m=d(`gpuCanvas`),h=d(`status`),g=d(`runBenchmark`),_=d(`benchmarkResult`),v=d(`recordHumanStroke`),y=d(`playHumanStroke`),b=d(`humanStrokeResult`),x=d(`layerFormat`),S=`webgpu-brush-engine.human-stroke.v1`,C=`/api/human-stroke`,w=new u(m,{onStatus(e,t){h.textContent=e,h.className=`status ${t===`working`?``:t}`},onStats(e){K(e)}}),T=null,E=null,D=!1,O=!1,k=!0,A=!1;function j(){return{color:d(`brushColor`).value,size:f(`brushSize`),spacingPercent:f(`spacing`),count:f(`count`),flow:f(`flow`)/100,hardness:f(`hardness`)/100,blendIntensity:f(`blendIntensity`),blendMode:d(`blendMode`).value,jitterMaster:f(`jitterMaster`)/100,hueJitterDegrees:f(`hueJitter`),saturationJitter:f(`saturationJitter`)/100,lightnessJitter:f(`lightnessJitter`)/100,darknessJitter:f(`darknessJitter`)/100,jitterPerCopy:d(`jitterPerCopy`).checked,positionJitterLateral:f(`positionJitterLateral`)/100,positionJitterLinear:f(`positionJitterLinear`)/100,pressureSize:f(`pressureSize`)/100,pressureOpacity:f(`pressureOpacity`)/100}}function M(){d(`brushSizeOut`).value=`${f(`brushSize`).toFixed(0)} px`,d(`spacingOut`).value=`${f(`spacing`).toFixed(2)}%`,d(`countOut`).value=f(`count`).toFixed(0),d(`flowOut`).value=`${f(`flow`).toFixed(1).replace(`.0`,``)}%`,d(`hardnessOut`).value=`${f(`hardness`).toFixed(0)}%`,d(`blendIntensityOut`).value=`${f(`blendIntensity`).toFixed(2)}×`,d(`jitterMasterOut`).value=`${f(`jitterMaster`).toFixed(0)}%`,d(`hueJitterOut`).value=`${f(`hueJitter`).toFixed(0)}°`,d(`saturationJitterOut`).value=`${f(`saturationJitter`).toFixed(0)}%`,d(`lightnessJitterOut`).value=`${f(`lightnessJitter`).toFixed(0)}%`,d(`darknessJitterOut`).value=`${f(`darknessJitter`).toFixed(0)}%`,d(`positionJitterLateralOut`).value=`${f(`positionJitterLateral`).toFixed(0)}%`,d(`positionJitterLinearOut`).value=`${f(`positionJitterLinear`).toFixed(0)}%`,d(`pressureSizeOut`).value=`${f(`pressureSize`).toFixed(0)}%`,d(`pressureOpacityOut`).value=`${f(`pressureOpacity`).toFixed(0)}%`,d(`benchmarkStampsOut`).value=p(f(`benchmarkStamps`))}function N(){M(),w.setBrushSettings(j())}function P(e){return`${(e/1e3).toFixed(2)} s`}function F(e){if(typeof e!=`object`||!e)return null;let t=e;return t.version!==1||!t.settings||!Array.isArray(t.points)||t.points.length===0?null:t}function I(){try{let e=window.localStorage.getItem(S);return e?F(JSON.parse(e)):null}catch{return null}}function L(){try{window.localStorage.removeItem(S)}catch{}}async function R(){let e=await fetch(C,{cache:`no-store`});if(e.status===404)return null;if(!e.ok)throw Error(`Impossibile caricare il tratto umano di riferimento.`);return F(await e.json())}async function z(e){let t=await fetch(C,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(e)});if(t.status===409){let e=F(await t.json());if(e)return e}if(!t.ok)throw Error(`Impossibile fissare il tratto umano di riferimento.`);let n=F(await t.json());if(!n)throw Error(`Il tratto umano salvato non è valido.`);return n}async function B(){k=!0,W();try{let e=await R();if(e){T=e,L(),b.textContent=G(e);return}let t=I();if(t){b.textContent=`Fissaggio del tratto che avevi già registrato…`,T=await z(t),L(),b.textContent=G(T);return}b.textContent=`Nessun tratto di riferimento: registralo una sola volta.`}catch(e){b.textContent=e instanceof Error?e.message:String(e)}finally{k=!1,W()}}function V(e,t){d(e).value=String(t)}function H(e){V(`brushColor`,e.color),V(`brushSize`,e.size),V(`spacing`,e.spacingPercent),V(`count`,e.count),V(`flow`,e.flow*100),V(`hardness`,e.hardness*100),V(`blendIntensity`,e.blendIntensity),V(`blendMode`,e.blendMode),V(`jitterMaster`,e.jitterMaster*100),V(`hueJitter`,e.hueJitterDegrees),V(`saturationJitter`,e.saturationJitter*100),V(`lightnessJitter`,e.lightnessJitter*100),V(`darknessJitter`,e.darknessJitter*100),d(`jitterPerCopy`).checked=e.jitterPerCopy,V(`positionJitterLateral`,e.positionJitterLateral*100),V(`positionJitterLinear`,e.positionJitterLinear*100),V(`pressureSize`,e.pressureSize*100),V(`pressureOpacity`,e.pressureOpacity*100),N()}function U(){return V(`brushSize`,750),V(`spacing`,1),V(`count`,16),V(`flow`,100),V(`hardness`,100),V(`blendIntensity`,4),V(`jitterMaster`,100),V(`hueJitter`,180),V(`saturationJitter`,100),d(`jitterPerCopy`).checked=!0,V(`positionJitterLateral`,100),V(`positionJitterLinear`,100),V(`pressureSize`,0),V(`pressureOpacity`,0),N(),j()}function W(){v.disabled=k||A||O||!!T,v.textContent=D?`Annulla registrazione tratto`:T?`Tratto umano fissato`:`Registra tratto umano`,y.disabled=!T||k||A||O}function G(e){let t=e.points.at(-1)?.timeMs??0;return[`Tratto salvato: ${p(e.points.length)} campioni`,`durata ${P(t)}`,`size ${e.settings.size.toFixed(0)} px`,`Count ${e.settings.count}`].join(` · `)}for(let e of[`brushColor`,`brushSize`,`spacing`,`count`,`flow`,`hardness`,`blendIntensity`,`blendMode`,`jitterMaster`,`hueJitter`,`saturationJitter`,`lightnessJitter`,`darknessJitter`,`jitterPerCopy`,`positionJitterLateral`,`positionJitterLinear`,`pressureSize`,`pressureOpacity`])d(e).addEventListener(`input`,N),d(e).addEventListener(`change`,N);d(`benchmarkStamps`).addEventListener(`input`,M),d(`clearLayer`).addEventListener(`click`,()=>w.clear()),d(`fitView`).addEventListener(`click`,()=>w.fitView()),d(`zoomIn`).addEventListener(`click`,()=>w.zoomBy(1.35)),d(`zoomOut`).addEventListener(`click`,()=>w.zoomBy(1/1.35)),x.addEventListener(`change`,async()=>{let e=x.value;x.disabled=!0;try{await w.setLayerFormat(e)}catch{x.value=w.getStats().layerFormat}finally{x.disabled=!1}}),g.addEventListener(`click`,async()=>{g.disabled=!0,_.textContent=`Benchmark in esecuzione sulla GPU…`;try{let e=await w.runBenchmark(f(`benchmarkStamps`));_.textContent=[`${p(e.baseStamps)} base stamps`,`${p(e.logicalCopies)} copie logiche`,`CPU submit ${e.cpuSubmitMs.toFixed(2)} ms`,`GPU completion ${e.gpuCompletionMs.toFixed(2)} ms`,`copertura teorica ${(e.estimatedCoveredFragments/1e6).toFixed(1)} Mpx`,e.strategy].join(` · `)}catch(e){_.textContent=e instanceof Error?e.message:String(e)}finally{g.disabled=!1}}),v.addEventListener(`click`,()=>{k||A||O||E||T||(D=!D,D?(U(),b.textContent=`Preset umano applicato. Disegna ora una sola pennellata sul canvas.`):b.textContent=T?G(T):`Registrazione annullata.`,W())}),y.addEventListener(`click`,()=>{ne()});function K(e){d(`fpsStat`).textContent=`${e.fps}`,d(`cpuStat`).textContent=`${e.lastCpuFrameMs.toFixed(2)} ms`,d(`stampStat`).textContent=p(e.totalBaseStamps),d(`avoidedStat`).textContent=p(e.avoidedLogicalDraws),d(`memoryStat`).textContent=`${e.layerMemoryMiB} MiB`,d(`gpuStat`).textContent=e.gpuLabel}function q(e,t){let n=U(),r=w.toLayerPoint(t);E={settings:n,startTimestamp:e.timeStamp,points:[{...r,timeMs:0}]},b.textContent=`Registrazione in corso…`}function ee(e,t){let n=E;if(n)for(let r=0;r<t.length;r+=1){let i=n.points[n.points.length-1]?.timeMs??0,a=Math.max(i,e[r].timeStamp-n.startTimestamp,0);n.points.push({...w.toLayerPoint(t[r]),timeMs:a})}}async function te(e){let t=E;if(E=null,D=!1,t&&e&&t.points.length>1){let e={version:1,capturedAt:new Date().toISOString(),settings:t.settings,points:t.points};A=!0,b.textContent=`Fissaggio permanente del tratto di riferimento…`,W();try{T=await z(e),L(),b.textContent=G(T)}catch(e){b.textContent=e instanceof Error?e.message:String(e)}finally{A=!1}}else t&&(b.textContent=`Tratto troppo breve: registra una pennellata con almeno un movimento.`);W()}async function ne(){let e=T;if(!(!e||O)){O=!0,g.disabled=!0,W(),H(e.settings),b.textContent=`Riproduzione del tratto umano in corso…`;try{await w.waitForIdle(),w.resetStrokeRandomSeed(),w.clear(),await w.waitForIdle();let t=w.getStats(),n=performance.now(),r=e.points[e.points.length-1],i=1;w.beginStrokeAtLayer(e.points[0]),await new Promise(t=>{let r=a=>{let o=a-n,s=[];for(;i<e.points.length&&e.points[i].timeMs<=o;)s.push(e.points[i]),i+=1;if(s.length>0&&w.extendStrokeAtLayer(s),i<e.points.length){requestAnimationFrame(r);return}w.endStroke(),t()};requestAnimationFrame(r)});let a=performance.now();await w.waitForIdle();let o=performance.now(),s=w.getStats(),c=Math.max(0,s.totalBaseStamps-t.totalBaseStamps),l=c*e.settings.count;b.textContent=[`Tratto ${P(r.timeMs)}`,`${p(e.points.length)} campioni`,`${p(c)} stamps base`,`${p(l)} copie fisiche`,`coda GPU ${Math.max(0,o-a).toFixed(2)} ms`,`CPU frame ${s.lastCpuFrameMs.toFixed(2)} ms`].join(` · `)}catch(e){b.textContent=e instanceof Error?e.message:String(e)}finally{O=!1,g.disabled=!1,W()}}}function re(e){return e.pointerType===`mouse`?1:e.pressure>0?e.pressure:e.pointerType===`pen`?.5:.65}function J(e){return{clientX:e.clientX,clientY:e.clientY,pressure:re(e)}}var Y=null,X=null,Z=0,Q=0;m.addEventListener(`pointerdown`,e=>{if(Y===null)if(e.preventDefault(),Y=e.pointerId,X=e.shiftKey||e.button===1||e.button===2?`pan`:`paint`,m.setPointerCapture(e.pointerId),X===`pan`)m.classList.add(`panning`),Z=e.clientX,Q=e.clientY;else{let t=J(e);D&&q(e,t),w.beginStroke(t)}}),m.addEventListener(`pointermove`,e=>{if(e.pointerId!==Y||X===null)return;if(e.preventDefault(),X===`pan`){w.panByClientDelta(e.clientX-Z,e.clientY-Q),Z=e.clientX,Q=e.clientY;return}let t=e.getCoalescedEvents?.()??[],n=t.length>0?t:[e],r=n.map(J);ee(n,r),w.extendStroke(r)});function $(e){e.pointerId===Y&&(X===`paint`&&(w.endStroke(),te(e.type===`pointerup`)),m.classList.remove(`panning`),X=null,Y=null)}m.addEventListener(`pointerup`,$),m.addEventListener(`pointercancel`,$),m.addEventListener(`lostpointercapture`,$),m.addEventListener(`contextmenu`,e=>e.preventDefault()),m.addEventListener(`wheel`,e=>{e.preventDefault();let t=Math.exp(-e.deltaY*.0015);w.zoomBy(Math.min(2,Math.max(.5,t)),e.clientX,e.clientY)},{passive:!1}),new ResizeObserver(()=>w.resizeCanvas()).observe(m),M(),w.setBrushSettings(j()),W(),B(),w.initialize().catch(e=>{let t=window.isSecureContext?``:` WebGPU richiede HTTPS oppure localhost; un indirizzo LAN in HTTP non è sufficiente.`;h.textContent=`${e instanceof Error?e.message:String(e)}${t}`,h.className=`status error`,g.disabled=!0}),window.setInterval(()=>K(w.getStats()),500);