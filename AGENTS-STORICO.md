...
## Blend dry: baseline iPhone run #76 e batch grande da 8

La run iPhone `#76` è la baseline del replay Blend dry pubblicato. Usa il
fingerprint `18982412`, size `750 px`, spacing richiesto `1%`, flow e hardness
`100%`, Paint `0%`, Stretch `20%`, Circle e Grain Off. Lo scratch era `cold` e
passa da `0` a circa `47,55 MiB`; il canvas è `860×1454`, la GPU è Apple e il
replay produce `2839` segmenti dry.

La run mostra frame regolari ma un forte ritardo visivo: `59,69 FPS`, intervallo
p95 `17 ms`, un solo frame oltre `20 ms` e CPU frame p95 `2 ms`, ma input
completato in circa `6851 ms`, coda dopo l'input `17006 ms` e presentazione
finale a `23870 ms`. I `2839` segmenti sono stati distribuiti su `1420` frame,
con `largestBatchStamps: 2`. Il campo coda include il drenaggio della FIFO Blend
un frame alla volta prima dell'ultimo `onSubmittedWorkDone`; non rappresenta
quindi diciassette secondi di solo lavoro GPU.

La causa isolata è il limite interattivo per size oltre `512 px`: due batch per
frame permettono circa `120` segmenti/s su uno schermo a `60 Hz`, mentre la
traccia #76 ne genera circa `415/s`. Il motore non può seguire il dito anche se
ogni singolo frame rispetta il refresh.

L'esperimento successivo cambia soltanto quel limite da `2` a `8`, identificato
da `DRY_BLEND_LARGE_BRUSH_BATCH_LIMIT = 8`. Tutte le soglie fino a `512 px`
restano invariate. Non cambiano planner, numero o ordine FIFO dei segmenti,
uniform, shader, pickup, carrier, pass gather/mask/deposit/scatter, size,
spacing, flow, hardness, Paint o Stretch. Gli otto batch vengono codificati
nello stesso ordine e ognuno conserva i propri cinque render pass; cambia
soltanto quanti batch consecutivi vengono inviati prima della presentazione.

La prima run iPhone valida con questo esperimento deve mostrare
`largestBatchStamps: 8` e va confrontata direttamente con la #76. Otto batch
forniscono un limite teorico di circa `480` segmenti/s a `60 Hz`, sufficiente
per la traccia, ma il risultato reale dipende dal costo GPU dei batch da
`750 px`. Mantenere l'intervento soltanto se coda finale e inseguimento del dito
migliorano senza una regressione in FPS, intervallo p95 o risultato visivo. Non
presentare WebGPU come più veloce del port WebGL prima di questa misura.

Questo esperimento è stato superato prima di una run iPhone valida: sul
desktop dell'utente il tratto continuava a inseguire il dito con la GPU sotto
al `5%` di utilizzo, sintomo che il collo di bottiglia non era il lavoro
raster ma l'overhead CPU dei render pass. Il limite da `8` e il tetto per size
sono stati sostituiti dai due passi seguenti.

## Blend dry: drenaggio a budget di pixel — sostituisce i tetti per size

I tetti interattivi per size (`64/32/24/8/4/8` batch per frame) erano copiati
da `smudgeFrameBatchLimit` del sidecar WebGL. Con segmenti dal `6%` della size
(`6 px` a size `100`) il throughput massimo era `sizeLimit × passo` per frame:
a size `100` circa `48 px` di tratto per frame, cioè `~2900 px/s` a `60 Hz`.
Sul layer `4096²` visto a zoom fit (`~0,17×`) il puntatore supera facilmente
quella velocità in pixel documento; l'eccesso si accumulava in
`pendingBlendBatches` e veniva rigiocato nei frame successivi, da cui il
ritardo visivo. Nella vecchia app WebGL gli stessi tetti non si notavano
perché il documento era tipicamente a zoom `~1:1`.

La selezione per frame ora usa un budget di pixel-pass invece di un conteggio
fisso: `DRY_BLEND_FRAME_PIXEL_BUDGET = 24_000_000`, costo per batch
`readRect.width × readRect.height × 2`, tetto assoluto
`DRY_BLEND_MAX_BATCHES_PER_FRAME = 256`. Le size piccole e medie drenano
quindi fino a `256` segmenti per frame (`~1500 px` di tratto a size `100`),
mentre le ROI da megapixel delle size grandi restano protette dal budget.
`submitBlendImmediate` spezza il gruppo in chunk da
`renderer.maximumBatchesPerSubmit` e fonde i dirty rect; il tetto di
sicurezza di `flushPendingWorkBeforeSettingsChange` assume nel caso peggiore
un solo batch per frame. `DRY_BLEND_LARGE_BRUSH_BATCH_LIMIT` è stato rimosso.
FIFO, planner, passo `6%`, ordine dei segmenti e pixel non cambiano.

## Blend dry: renderer compute v3 `dry-blend-webgpu-v3-compute-fused-sweep`

Alzare il drenaggio non bastava: con 5 render pass per segmento, `100+`
segmenti per frame significavano centinaia di `beginRenderPass`, e il costo
CPU per pass (validazione ed encoding) saturava il frame mentre la GPU restava
quasi ferma. Il renderer è stato riscritto in compute mantenendo identico il
modello a pigmento trasportato:

1. i segmenti consecutivi del submit vengono raggruppati finché la ROI unita
   sta nello scratch `1664²`;
2. per ogni gruppo un solo compute pass esegue: un dispatch di gather della
   ROI unita dal layer, poi per ogni segmento un dispatch di pickup (una
   workgroup `8×8`, un thread per tap, riduzione in memoria workgroup) e un
   dispatch di deposit ritagliato alla writeRect del segmento;
3. un solo render pass di scatter per gruppo riporta sul layer i pixel con
   coverage unione positiva, con scissor sulla writeRect unita.

La mask swept non è più un pass: la coverage è calcolata inline nel deposit.
Compute non ha `fwidth`/`dpdx`, quindi gli antialias usano i valori analitici
(bordo del cerchio `1,6 / radius`, gate dello spacing `1,0`, gradienti grain
analitici per mip e `textureSampleGrad`). Lo stato scratch è un storage
buffer `vec4<f32>` con stride di riga fisso (`~44,3 MiB`), affiancato da un
buffer di coverage unione `f32` (`~11,1 MiB`) azzerato dal gather; il deposit
fa read-modify-write in place senza ping-pong né copie. Il carrier è un ring
di `4096` slot in un buffer (`64 KiB`): il segmento `i` legge lo slot corrente
e scrive il successivo, `beginStroke` azzera cursore e validità. Le vecchie
texture di stato, mask e carrier non esistono più; la memoria scratch totale è
`~55,5 MiB`.

Un segmento costa così due dispatch (microsecondi di CPU) invece di cinque
render pass, e gather/scatter si pagano una volta per gruppo. Dentro un
gruppo lo stato resta in float pieno invece di riquantizzarsi nel layer
`rgba8unorm` a ogni segmento: meno banding accumulato, stessa semantica.

Restano identici: planner e passo `6%`, ordine FIFO, curve `sqrt(Stretch)` e
`Paint²`, flow/hardness/spacing nella coverage, pickup `8×8` pesato con la
strategia di bordo
`exclude-outside-document-preserve-carrier` (tap fuori documento esclusi,
lookup clampato, trasporto del carrier sui passi fuori canvas), carica Paint,
alpha floor, gate del coverage unione sullo scatter, reset del carrier per
azione di history, neutralità della pressione e tutto il percorso Paint.

Verifiche: `npm run blend:verify` (assert aggiornate su budget, tetto `256`,
costo `×2` e chunking), TypeScript, avvio senza errori WGSL e tratto Blend
funzionante su GPU NVIDIA Ampere. L'utente conferma sul proprio desktop che il
tratto ora segue il puntatore molto meglio. Non esiste ancora una run iPhone
canonica del renderer v3: la prossima run Blend va confrontata con la `#76`
sapendo che `largestBatchStamps` può arrivare a `256` e che `passCount` conta
ora `2` pass per gruppo più l'eventuale clear, quindi non è confrontabile con
i conteggi a 5 pass delle build precedenti. Nota attesa: l'utilizzo GPU del
Blend resta basso anche a regime — il lavoro è di pochi megapixel per frame;
non usarlo come indicatore di regressione.
